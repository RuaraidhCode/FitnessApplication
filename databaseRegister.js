...
app.post('/api/updateHeight', async (req, res) => {
  ...
  app.get('/api/userGoals', async (req, res) => {
    ...
    try {
      const Qresult = await client.query("SELECT * FROM goals WHERE user_id = $1 AND goal_status = 'In Progress';", [user_id]);
      const goalHistory = Qresult.rows;
  ...
  console.log("Trying to do backend")
    try {
      await client.query(
        `
        UPDATE goals 
  ...
  app.post('/api/deleteGoal', async (req, res) => {
    const { goalId } = req.body;
  console.log("Trying to do backend")
    try {
      await client.query(
...
app.post('/api/initialiseWeekStartDate', async (req, res) => {
  const { workout, userInfo } = req.body;
  console.log('Received workout from frontend:', workout);
  ...
  try {
    ...
    // Insert workout data into workout_history table
    const insertWorkoutHistoryQuery = `
      INSERT INTO workout_history (user_id, workout_date, week_start_date, duration, workout_count)
      VALUES ($1, NOW(), NOW(), $2, (SELECT COALESCE(MAX(workout_count), 0) + 1 FROM workout_history WHERE user_id = $1))
      RETURNING workout_history_id;    
    `;
...
app.get('/api/weekStartDate', async (req, res) => {
  const { user_id } = req.query;
  try {
    const Qresult = await client.query('SELECT week_start_date FROM workout_history WHERE user_id = $1 AND week_start_date IS NOT NULL;', [user_id]);
    const weekStartDate = Qresult.rows;


    res.json(weekStartDate);
  } catch (error) {
    console.log('Error:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
...
app.post('/api/addCompetitor', async (req, res) => {
  ...
  console.log('Received createStatsPackage from frontend:');
  ...
  try {
    ...
    const createStatsPackageQuery = `
      INSERT INTO weekly_progress (comp_id, user_id, comp_name, comp_category, team_id, weight, weight_changes, points)
      VALUES ($1, $2, $3, $4, $5, $6, ARRAY[$7]::NUMERIC[], $8);
    `;
...
app.get('/api/allCompetitionHistory', async (req, res) => {
  const { user_id } = req.query; 
  try {
    const Qresult = await client.query('SELECT * FROM competition_history WHERE user_id = $1;', [user_id]);
    const leaderboardList = Qresult.rows;
...
app.get('/api/usersCompetitionHistory', async (req, res) => {
  const { user_id } = req.query; 
  try {
    const Qresult = await client.query('SELECT * FROM solo_final_standings WHERE user_id = $1;', [user_id]);
    const leaderboardList = Qresult.rows;
...
app.get('/api/leaderboardCompetitionHistory', async (req, res) => {
  const { compId } = req.query; 
  try {
    const Qresult = await client.query('SELECT * FROM competition_history WHERE comp_id = $1;', [compId]);
    const leaderboardList = Qresult.rows;
console.log('backend cop id ---> ', compId)
...
app.get('/api/leaderboardSoloHistory', async (req, res) => {
  const { compId } = req.query; 
  try {
    const Qresult = await client.query('SELECT * FROM solo_final_standings WHERE comp_id = $1;', [compId]);
    const leaderboardList = Qresult.rows;
console.log('backend cop id ---> ', compId)
...
app.get('/api/leaderboardTeamHistory', async (req, res) => {
  const { compId } = req.query; 
  try {
    const Qresult = await client.query('SELECT * FROM team_final_standings WHERE comp_id = $1;', [compId]);
    const leaderboardList = Qresult.rows;
console.log('backend cop id ---> ', compId)
  ...
  try {
    ...
    if (rows.length === 1) {
      ...
      if (completed === true) {
        ...
        console.log('Competition is already marked as completed.');
        return;
      }
      ...
      if (is_marked_completed === false) {
        ...
        const insertCompetitionHistoryQuery = `
        INSERT INTO competition_history (comp_id, user_id, comp_name, username, weight, weight_changes, points, team_id, team_name, team_colour, comp_end_date, comp_start_date)
        SELECT wp.comp_id, wp.user_id, wp.comp_name, ru.username, wp.weight, wp.weight_changes, wp.points, NULL, NULL, NULL, c.comp_end_date, c.comp_start_date
        FROM weekly_progress wp
        JOIN register_user ru ON wp.user_id = ru.user_id
        JOIN competition c ON wp.comp_id = c.comp_id
        WHERE wp.comp_id = (SELECT comp_id FROM weekly_progress WHERE user_id = $1);
      ...
      await client.query(insertCompetitionHistoryQuery, [user_id]);
      console.log('Successfully populated competition_history.');
  ...
  try {
    ...
    if (rows.length === 1) {
      ...
      if (is_marked_completed === false) {
        ...
        const insertCompetitionHistoryQuery = `
        INSERT INTO competition_history (comp_id, user_id, comp_name, comp_category, username, weight, weight_changes, points, team_id, team_name, team_colour, comp_end_date, comp_start_date)
        SELECT wp.comp_id, wp.user_id, wp.comp_name, 'Solo', ru.username, wp.weight, wp.weight_changes, wp.points, t.team_id, t.team_name, t.team_colour, c.comp_end_date, c.comp_start_date
        FROM weekly_progress wp
        JOIN team t ON wp.team_id = t.team_id
        JOIN register_user ru ON wp.user_id = ru.user_id
        JOIN competition c ON wp.comp_id = c.comp_id
...
app.post('/api/removeOldLeaderboard', async (req, res) => {
  ...
  try {
    ...
    if (soloRows.length === 0 || soloRows[0].team_id === null) {
      ...
      const removeSoloProgressQuery = 'DELETE FROM weekly_progress WHERE user_id = $1 AND team_id IS NULL';
      await client.query(removeSoloProgressQuery, [user_id]);

      console.log('Successfully removed user from solo weekly_progress.');
      ...
      const removeTeamProgressQuery = 'DELETE FROM weekly_progress WHERE user_id = $1 AND team_id IS NOT NULL';
      await client.query(removeTeamProgressQuery, [user_id]);

      console.log('Successfully removed user from team weekly_progress.');
...
app.get('/api/recieveExperience', async (req, res) => {
  const { user_id } = req.query;
  try {
    const Qresult = await client.query('SELECT * FROM users WHERE user_id = $1;', [user_id]);
    const userList = Qresult.rows;
...
app.post('/api/earnXp', async (req, res) => {
  const { user_id, xpAmount } = req.query;
  try {
    const Qresult = await client.query('UPDATE users SET experience = experience + $2, total_experience = total_experience + $2 WHERE user_id = $1 RETURNING *;', [user_id, xpAmount]);
    const updatedUser = Qresult.rows[0]; // Assuming you're updating one user

    console.log('Earning xp for user_id ---> ', user_id);
    console.log('Earning xp for xpAmount ---> ', xpAmount);

    res.json(updatedUser); // Sending back the updated user data
  } catch (error) {
    console.log('Error:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
