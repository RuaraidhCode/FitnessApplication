...
const Leaderboard = () => {
  ...
  const isCompetitionEnded = currentDate >= new Date(leaderboardListData.find(entry => entry.comp_id === compId)?.comp_end_date);
...
const completedTeamCompLeaderboard = () => {
  axios.post(`${BASE_URL}/api/api/setCompletedTeamCompLeaderboard`, { user_id: userInfo.user_id })
    .then((response) => {
      console.log('Successfully sent the completed comp leaderboard POST request');
      // Handle the response if needed
    })
  ...
  const handleWeightOptionPress = (selectedWeight) => {
    ...
    const newUserPoints = ((startingWeight - selectedWeight) * 100).toFixed(0);
      ...
      >
             <View style={styles.startingStatsModalContainer}>
               <View style={styles.startingStatsModalContent}>
                   <Text style={styles.startingStatsTitle}>Week {weekNumber}</Text>
